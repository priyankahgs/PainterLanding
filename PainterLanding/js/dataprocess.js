...
function isAlphabetKeyState(evt, txt) {
    ...
    if (!(charCode >= 65 && charCode <= 122) && (charCode != 32 && charCode != 0) && charCode > 31 || (charCode >= 91 && charCode <= 96)) {
        if (txt == 'state') {
            $('#spnState').text('Special characters/numbers not allowed');
        }
        return false;
    }
    $('#spnState').text('');
...
function isAlphabetKeyGst() {
    $("#txtexpGst").keypress(function (e) {
        ...
        if (!isValid) {
            $("#spnGst").html("Only Alphabets and Numbers allowed");
        }
